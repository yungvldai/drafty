...
export default {
  methods: {
    isBlockUseKeyboard(block) {
      ...
      if (found && found.options.useKeyboard) return true;
      return false;
    },
    ...
    handleEnter(caretPos) {
      ...
      this.$nextTick(() => { // pass render to focus
        this.NEXT['editor-only'].native.focus();
      });
    },
    handleBackspace() {
    ...
    navPrev() {
      ...
      if (found) {
        found['editor-only'].native.focus();
        caretUtils.setCaretPos(found['editor-only'].native, this.PREV.data.content.length);
      }
    },
    navNext() {
